...
def _baseline_fields(registry: MigrationRegistry, down_revision: str | None) -> dict[str, str]:
    """Reconstruct the field schema as of ``down_revision`` (the autogenerate baseline).

    A revision that carries a ``fields`` snapshot *is* the schema after it ran. A
    merge (or any hand-written revision) carries ``fields = None``, so its schema
    is reconstructed as the **union of the schemas after each parent** — newest
    snapshot per branch. This matters after a merge: picking a single branch's
    snapshot would diff the new code schema against only one side, generating
    spurious adds and — worse — missing drops/coercions for fields that live only
    on the other branch.
    """
    if down_revision is None:
        return {}
    # The registry is validated acyclic at construction, so this recursion
    # terminates (no ``seen`` guard needed); migration histories are shallow.
    return _schema_after(registry, down_revision)


def _schema_after(registry: MigrationRegistry, revision: str) -> dict[str, str]:
    """Field schema after ``revision``: its own snapshot, else the union of parents'."""
    mig = registry.get(revision)
    if mig.fields is not None:
        return dict(mig.fields)
    merged: dict[str, str] = {}
    for parent in mig.parents:
        merged.update(_schema_after(registry, parent))
    return merged
